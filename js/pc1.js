...
async function main(img, img2) {
  ...
  const vertSrc = await fetch("glsl/pc1.vert").then((resp) => resp.text());
  const fragSrc = await fetch("glsl/pc1.frag").then((resp) => resp.text());
  const shader = wu.createProgramFromSources(gl, [vertSrc, fragSrc]);

  const cam = new cg.Cam([0, 1.5, 10]);
  const mesh = createCube(gl, shader, 1.0);
  const mesh1 = createCube(gl, shader, 1.0);
  const tfactors = new Float32Array([0, 0, 0]);
  const tfactors1 = new Float32Array([1, 0, 1]);
  const rotationAxis = new Float32Array([0, 1, 0]);
  const rotationAxis1 = new Float32Array([0, 0, 2]);
  ...
  function render(elapsedTime) {
    ...
    tfactors1[0] = Math.sin(elapsedTime);
    tfactors1[1] = Math.cos(elapsedTime);
    mat4.translate(model, model, tfactors);
    mat4.translate(model1, model1, tfactors1);
    mat4.rotate(model, model, theta, rotationAxis);
    mat4.rotate(model1, model1, theta, rotationAxis1);

    mat4.perspective(projection, cam.zoom, aspect, 0.1, 100);

    gl.useProgram(shader);
    gl.uniformMatrix4fv(modelLoc, false, model1);
  ...
  document.addEventListener("keydown", (e) => {
    ...
    else if (e.key === "d") cam.processKeyboard(cg.LEFT, deltaTime);
    else if (e.key === "s") cam.processKeyboard(cg.BACKWARD, deltaTime);
    else if (e.key === "a") cam.processKeyboard(cg.RIGHT, deltaTime);
  });
...
fondo = "textures/salpicadura.jpg"
fondo1 = "textures/rocamagma.jpg"
